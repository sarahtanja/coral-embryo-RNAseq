...
```{r}
plotly::subplot(plot_top3, nrows = 2, margin = 0.05) %>% 
  layout(title = 'Top 3 genes', shareX = T, showlegend=FALSE)
```
...
sig_cat : List of the 44 genes that show significant interactions between leachate and stage (saved above) LRT_sig_genes : List of LRT significant genes, LFC, & padj values (saved above) Wald_sig_genes : List of Wald test results, LFC & padj values for each interaction contrast vsd_sig : Variance stabilized count matrix of genes from significant list all_genes : List of all genes in the dataset (the filtered input to the DESeq object) which will serve as a reference for functional enrichment

```{r}
Wald_sig_genes <- int_wald_df_sig

write.csv(Wald_sig_genes, "../../output/06_deg/interactions/Wald_sig_genes.csv")
```
...
write.csv(vsd_sig, "../../output/06_deg/interactions/vsd_sig.csv")
```
...
write.csv(all_genes, "../../output/06_deg/interactions/all_genes.csv")
```
