...
```{r, eval=TRUE}
dds_cat <- DESeq(dds_cat, test = "LRT", reduced = ~0 + stage + leachate)
dds_con <- DESeq(dds_con, test = "LRT", reduced = ~0 + stage + leachate_mgL)
```
...
write.csv(LRT_sig_genes, "../../output/06_deg/interactions_zerointercept/LRT_sig_genes.csv")
```
...
```{r}
write.csv(sig_cat, "../../output/06_deg/interactions_zerointercept/sig_interaction_genes.csv")
```
...
for (i in 1:nrow(all_info)) {
  ...
  plots[[i]] <- ggplotly(p)
    ...
    # Save to PNG
  ggsave(
    filename = paste0("../../output/06_deg/interactions_zerointercept/gene_plots/", padj_rank, ".png"),
...
png("../../output/06_deg/interactions_zerointercept/int_wald_heatmap.png", width = 10, height = 12, units = "in", res = 600)
...
write_csv(Wald_sig_genes, "../../output/06_deg/interactions_zerointercept/Wald_sig_genes.csv")
```
...
write_csv(vsd_sig, "../../output/06_deg/interactions_zerointercept/vsd_sig.csv")
```
...
write_csv(all_genes, "../../output/06_deg/interactions_zerointercept/all_genes.csv")
```
...
```{r}
write.csv(groups, "../../output/06_deg/interactions_zerointercept/clusters.csv")
```
...
ggsave(cluster_plot, filename="../../output/06_deg/interactions_zerointercept/cluster_plot.png", width=12, height=10)
```
# Summary & Next Steps
