...
### Plot distribution of unfiltered read counts 
...
```{r}
# Find the gene with maximum absolute logFC
extreme_gene_index <- which.max(abs(nl_leachate$table$logFC))
extreme_gene <- rownames(nl_leachate$table)[extreme_gene_index]

# View its details
nl_leachate$table[extreme_gene_index, ]
```

```{r}
counts <- y1$counts  # assuming y1 is your DGEList
counts[extreme_gene, ]
```
```{r}
head(nl_leachate$table)
```



```{r}
# Export diff expression data for transcripts with significant DE associated with leachate parameter
nl_leachate_sig <-
  topTags(
  nl_leachate,
  n = Inf, 
...
```{r}
writeLines(extreme_gene, "../output/09_deg_multifactorial/filtered_outlier_gene.txt")
```
...
gcm_filt <- gcm_filt %>%
  filter(rowSums(across(everything(), ~ . > 200)) >= 0.33 * ncol(gcm))
...
dds <- DESeqDataSetFromMatrix(gcm_filt,
...
DESeq2_result_names <- resultsNames(dds)

# Count DEGs due to interaction
DESeq2_results <- results(dds, lfcThreshold = 0, alpha = 0.05)
