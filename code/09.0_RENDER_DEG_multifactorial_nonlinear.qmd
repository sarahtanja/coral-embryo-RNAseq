...
All 63 samples across 12 treatments grouped by hours post fertilization ![](images/vsdPCA-01.png)
...
![MarineOmics biological coefficient of variation plot for comparison](marine-omics-plotBCV.jpg) \### Fit multifactorial design matrix
...
gcm_filt_des <- gcm_filt %>%
  filter(rowSums(across(everything(), ~ . > 15)) >= 0.8 * ncol(gcm_filt))
...
sig_leachate_counts <- dds_counts[rownames(dds_counts) %in% sig_leachate$gene, ]

sum(sig_leachate$gene %in% rownames(dds_counts))
dput(sig_leachate$gene)
```
