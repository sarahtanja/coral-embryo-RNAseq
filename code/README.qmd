...
Setup .gitignore file to ignore filetypes that will be large!
...
Execute this code in terminal prior to committing to add all files under 50M
...
## 1. Get raw FASTA files

-   files are already `demultiplexed`

-   files have a `.fasta.gz` zipped format

-   files must be checked to make sure there were no errors in the transfer process (this is done with `md5sum` )
...
-   Obtain reference genome assembly & GFF annotation file
    ...
    -   [GFF fixed](https://github.com/AHuffmyer/EarlyLifeHistory_Energetics/raw/master/Mcap2020/Data/TagSeq/Montipora_capitata_HIv3.genes_fixed.gff3.gz) from AHuffmyer, modified from a [GFF](http://cyanophora.rutgers.edu/montipora/Montipora_capitata_HIv3.genes.gff3.gz){style="font-size: 11.4pt;"} file from Rutgers
...
## 5. Conduct initial data exploration

-   

-   Filter gene count matrix to remove low read count genes

-   Visualize exploratory PCAs

-   Identify and remove outlier samples

-   Reformat metadata to match filtered gene count matrix

## 6. Identify differentially expressed genes (DEGs)

-   [DESeq-2](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html)
-   MarineOmics [Fitting Multifactorial Models of Differential Expression](https://marineomics.github.io/DGE_comparison_v2.html#Non-linear_effects:_example_in_edgeR)
