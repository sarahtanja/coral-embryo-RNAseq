...
```{r}
clusters <- read.csv("../../output/06_deg/interactions/clusters.csv")
```
...
::: callout-note
37 of the 44 significant genes were in the `EggNog_anno` dataframe!
:::
...
Drop NA GO term rows

```{r}
sig_genes_anno <- sig_genes_anno %>% drop_na(GOs)
...
annotation file `sig_genes_anno` dataframe now only contains genes that
were significant *and* have annotation information.
:::

Add column indicating cluster of expression pattern (1-4)

```{r}
sig_genes_anno <- inner_join(sig_genes_anno, clusters, by = "gene_id")
str(sig_genes_anno)
```
...
## Subset to interaction pattern clusters

```{r}
sig_genes_anno1 <- sig_genes_anno %>% 
  filter(leachate_cluster == 1)

sig_genes_anno2 <- sig_genes_anno %>% 
  filter(leachate_cluster == 2)

sig_genes_anno3 <- sig_genes_anno %>% 
  filter(leachate_cluster == 3)

sig_genes_anno4 <- sig_genes_anno %>% 
  filter(leachate_cluster == 4)

length(sig_genes_anno1$leachate_cluster)
length(sig_genes_anno2$leachate_cluster)
length(sig_genes_anno3$leachate_cluster)
length(sig_genes_anno4$leachate_cluster)
```
...
> 'close enough' pattern. We'll stick with the 4 patterns.
:::
...
ID.vector.all <- sig_genes_anno$gene_id # all sig interaction genes
ID.vector.1 <- sig_genes_anno1$gene_id # all sig interaction genes that fall into cluster 1
ID.vector.2 <- sig_genes_anno2$gene_id # all sig interaction genes that fall into cluster 2
ID.vector.3 <- sig_genes_anno3$gene_id # all sig interaction genes that fall into cluster 3
ID.vector.4 <- sig_genes_anno4$gene_id # all sig interaction genes that fall into cluster 4
...
ALL.vector <- ALL.vector[ALL.vector %in% EggNog_anno$gene_id] #filtered down to only 37 genes that are annotated

length(ALL.vector)
length(ID.vector.all)
length(ID.vector.1)
length(ID.vector.2)
length(ID.vector.3)
length(ID.vector.4)
...
::: callout-note
9563 out of the total 12564 genes in our dataset are annotated. 23 of
those genes are both significantly differentially expressed (due to
leachate:stage interaction) and have annotations.
:::
...
GO.terms.all <- sig_genes_anno %>%
  dplyr::select(gene_id, GOs)

GO.terms.1 <- sig_genes_anno %>%
  dplyr::select(gene_id, GOs, leachate_cluster) %>% 
  dplyr::filter(leachate_cluster == "1")

GO.terms.2 <- sig_genes_anno %>%
  dplyr::select(gene_id, GOs, leachate_cluster) %>% 
  dplyr::filter(leachate_cluster == "2")

GO.terms.3 <- sig_genes_anno %>%
  dplyr::select(gene_id, GOs, leachate_cluster) %>% 
  dplyr::filter(leachate_cluster == "3")

GO.terms.4 <- sig_genes_anno %>%
  dplyr::select(gene_id, GOs, leachate_cluster) %>% 
  dplyr::filter(leachate_cluster == "4")
```
...
GO.terms.all <- split_go(GO.terms.all)
...
gene.vector.all <- as.integer(ALL.vector %in% ID.vector.all)
names(gene.vector.all) <- ALL.vector

gene.vector.1 <- as.integer(ALL.vector %in% ID.vector.1)
names(gene.vector.1) <- ALL.vector

gene.vector.2 <- as.integer(ALL.vector %in% ID.vector.2)
names(gene.vector.2) <- ALL.vector

gene.vector.3 <- as.integer(ALL.vector %in% ID.vector.3)
names(gene.vector.3) <- ALL.vector

gene.vector.4 <- as.integer(ALL.vector %in% ID.vector.4)
names(gene.vector.4) <- ALL.vector

# Null probability weighting function (bias for gene length)
# nullp is a function from nadiadavidson/goseq
pwf.all <- nullp(gene.vector.all, bias.data = LENGTH.vector)
...
```{r}
length(intersect(names(gene.vector.all), GO.terms.all[[1]]))
```
...
GO.bp.all$bh_adjust <- p.adjust(GO.bp.all$over_represented_pvalue, method = "BH")
...
```{r}
# Write to csv
write_csv(GO.BP.all, file = "../../output/08_go/functional_enrichment_goseq_44int.csv")
write_csv(GO.BP.1, file = "../../output/08_go/functional_enrichment_goseq_44intclust1.csv")
write_csv(GO.BP.2, file = "../../output/08_go/functional_enrichment_goseq_44intclust2.csv")
write_csv(GO.BP.3, file = "../../output/08_go/functional_enrichment_goseq_44intclust3.csv")
write_csv(GO.BP.4, file = "../../output/08_go/functional_enrichment_goseq_44intclust4.csv")
```
...
simMatrix.all <- calculateSimMatrix(
  GO.BP.all$category, # the column of go terms
...
# Create named scores vector
scores.all <- setNames(
  -log10(pmax(GO.BP.all$bh_adjust, .Machine$double.xmin)),
  GO.BP.all$category
)
