...
- 37 of the 44 (84.1%) significant interaction genes were in the `EggNog_anno` dataframe!
...
annotation file `sig_genes_annofilt` dataframe now only contains genes that
were significant *and* have GO annotation information.
...
mapping step. I think it's because I included genes without GOs in the universe for bias correction. I've changed this so that I filter out any that lack GO terms. 
...
## Interaction
ID.vector.int <- sig_genes_af$gene_id # significant interaction genes

ID.vector.1 <- sig_genes_af1$gene_id # all sig interaction genes that fall into cluster 1
ID.vector.2 <- sig_genes_af2$gene_id # all sig interaction genes that fall into cluster 2
ID.vector.3 <- sig_genes_af3$gene_id # all sig interaction genes that fall into cluster 3
ID.vector.4 <- sig_genes_af4$gene_id # all sig interaction genes that fall into cluster 4

## Contrasts
ID.vector.lvc_cl <- sig_lvc_cl_af$gene_id # all sig genes low vs control cleavage contrast
ID.vector.mvc_cl <- sig_mvc_cl_af$gene_id # all sig genes mid vs control cleavage contrast
ID.vector.hvc_cl <- sig_hvc_cl_af$gene_id # all sig genes high vs control cleavage contrast
ID.vector.hvc_eg <- sig_hvc_eg_af$gene_id # all sig genes high vs control early gastrula contrast
