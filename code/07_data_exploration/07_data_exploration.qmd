...
-   Create initial DESeq Dataset Objects to use in exploratory data visualizations
...
load("../metadata/metadata_4.RData")
load("../metadata/metadata_9.RData")
load("../metadata/metadata_14.RData")
load("../output/06_tidyup/gcm_filt.RData")
load("../output/06_tidyup/gcm_filt_4.RData")
load("../output/06_tidyup/gcm_filt_9.RData")
load("../output/06_tidyup/gcm_filt_14.RData")
```

# Make [DESeq Dataset Objects from Count Matrix](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#count-matrix-input)

Create a DESeqDataSet design from gene count matrix and treatment conditions. Here we set the design to look at pollution exposure (C, L, M, H) to test for any differences in gene expression across the increasing exposure concentrations within an developmental stage (4, 9, or 14 hours post fertilization).
...
## Make interaction term design (`ddo_i`)

`~genotype + condition + genotype:condition`

```{r}
#Set DESeq2 design for 2 factors and their interaction
ddo_i <- DESeqDataSetFromMatrix(countData = gcm_filt,
                              colData = metadata,
                              design = ~ hpf +  pvc_leachate_level + hpf:pvc_leachate_level)
```
...
Check levels in both the `pvc_leachate_level` and `embryonic_phase` factors for the DESeq Object `ddo_i` is the object made with an interaction term)

```{r}
levels(ddo_i$pvc_leachate_level)
levels(ddo_i$hpf)
```
...
## Make DESeq Dataset Object (`ddo_4`) for 4hpf

```{r}
ddo_4 <- DESeqDataSetFromMatrix(countData = gcm_filt_4,
                              colData = metadata_4,
                              design = ~ pvc_leachate_level)
```

```{r}
levels(ddo_4$pvc_leachate_level)
```

## Make DESeq Dataset Object (`ddo_9`) for 9hpf

```{r}
ddo_9 <- DESeqDataSetFromMatrix(countData = gcm_filt_9,
                              colData = metadata_9,
                              design = ~ pvc_leachate_level)
```

```{r}
levels(ddo_9$pvc_leachate_level)
```

## Make DESeq Dataset Object (`ddo_14`) for 14hpf

```{r}
ddo_14 <- DESeqDataSetFromMatrix(countData = gcm_filt_14,
                              colData = metadata_14,
                              design = ~ pvc_leachate_level)
```

```{r}
levels(ddo_14$pvc_leachate_level)
```
...
>
> To do this we first need to calculate the size factors of our samples. This is a rough estimate of how many reads each sample contains compared to the others. In order to use VST (the faster log2 transforming process) to log-transform our data, the size factors need to be less than 4. - [A.Huffmyer Early Life History Energetics](https://github.com/AHuffmyer/EarlyLifeHistory_Energetics/blob/07ef97f446e4383b95bbcc78e5d0e8df1736d51e/Mcap2020/Scripts/TagSeq/DESeq2_Mcap_V3.Rmd#L206){style="font-size: 11.4pt;"}

```{r}
SF_ddo_i <- estimateSizeFactors(ddo_i)
print(sizeFactors(SF_ddo_i))
all(sizeFactors(SF_ddo_i))<4
```

## Full

```{r}
vsd_i <- vst(ddo_i, blind=FALSE)
rld_i <- rlog(ddo_i, blind=FALSE)
```

## 4hpf

```{r}
vsd_4 <- vst(ddo_4, blind=FALSE)
rld_4 <- rlog(ddo_4, blind=FALSE)
```

## 9hpf

```{r}
vsd_9 <- vst(ddo_9, blind=FALSE)
rld_9 <- rlog(ddo_9, blind=FALSE)
```

## 14hpf

```{r}
vsd_14 <- vst(ddo_14, blind=FALSE)
rld_14 <- rlog(ddo_14, blind=FALSE)
```
...
```{r}
ddo_i <- estimateSizeFactors(ddo_i)

df <- bind_rows(
  as.data.frame(log2(counts(ddo_i, normalized=TRUE)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
  as.data.frame(assay(vsd_i)[, 1:2]) %>% mutate(transformation = "vst"),
  as.data.frame(assay(rld_i)[, 1:2]) %>% mutate(transformation = "rlog"))
...
```{r}
ddo_4 <- estimateSizeFactors(ddo_4)

df <- bind_rows(
  as_data_frame(log2(counts(ddo_4, normalized=TRUE)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
...
```{r}
ddo_9 <- estimateSizeFactors(ddo_9)

df <- bind_rows(
  as_data_frame(log2(counts(ddo_9, normalized=TRUE)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
...
```{r}
ddo_14 <- estimateSizeFactors(ddo_14)

df <- bind_rows(
  as_data_frame(log2(counts(ddo_14, normalized=TRUE)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
...
# Exploratory priciple component analysis

::: callout-caution
USE THE DESEQ OBJECTS FOR EXPLORATORY DATA VISUALIZATION!
:::
...
```{r}
vsd_pca <- plotPCA(vsd_i, intgroup=c("pvc_leachate_level", "embryonic_stage"), returnData=TRUE)

percentVar <- round(100 * attr(vsd_pca, "percentVar"))

# Make the plot
vsdPCA <- ggplot(vsd_pca, aes(PC1, PC2, 
                              color=embryonic_stage,
...
ggsave("../output/07_exploration/vsd_i_PCA.png", plot = vsdPCA, width = 8, height = 6, dpi = 600)
```

```{r}
rld_pca <- plotPCA(rld_i, intgroup=c("pvc_leachate_level", "embryonic_stage"), returnData=TRUE)
...
# Save the plot as a png
ggsave("../output/07_exploration/rld_i_PCA.png", plot = rldPCA, width = 8, height = 6, dpi = 600)
```
...
pca_4 <- prcomp(t(gcm_filt_4))

```

```{r}
# Extract PC1 values
pc1_scores <- pca_4$x[, "PC1"]

# Convert PC1 scores to Z-scores
pc1_z <- scale(pc1_scores)

# Get samples with Z-score > |3|
outliers <- pc1_z[abs(pc1_z) > 3]
...
```{r}
pca_plot_4 <- plotPCA(vsd_4, intgroup=c("pvc_leachate_level", "embryonic_stage"), returnData=TRUE)

percentVar <- round(100 * attr(pca_plot_4, "percentVar"))

# Make the plot
PCAout4 <- ggplot(pca_plot_4, aes(PC1, PC2, 
                              color=embryonic_stage,
                              shape=pvc_leachate_level)) +
                              geom_text(aes(label = name), size = 3) +  # Add labels from the 'name' column
                              geom_point(size=3) +
...
::: callout-important
Although their z-scores are not greater than 3 standard deviations from the group, visually samples `131415L4` and `789C4`may be outliers in the cleavage phase
:::
...
gcm_filt_4_or <- gcm_filt_4_or %>%
  filter(rowSums(across(everything(), ~ . > 10)) >= 0.16 * ncol(gcm_filt_4_or))
...
gcm_filt_or <- gcm_filt_or %>%
  filter(rowSums(across(everything(), ~ . > 10)) >= 0.05 * ncol(gcm_filt_or))
...
```{r}
ddo_4_or <- DESeqDataSetFromMatrix(countData = gcm_filt_4_or,
                              colData = metadata_4_or,
                              design = ~ pvc_leachate_level)
```

```{r}
ddo_i_or <- DESeqDataSetFromMatrix(countData = gcm_filt_or,
                              colData = metadata_or,
                              design = ~ embryonic_stage +  pvc_leachate_level + embryonic_stage:pvc_leachate_level)
```

### Redo count transformations

```{r}
vsd_4_or <- vst(ddo_4_or, blind=FALSE)
rld_4_or <- rlog(ddo_4_or, blind=FALSE)
```

```{r}
ddo_4_or <- estimateSizeFactors(ddo_4_or)

df <- bind_rows(
  as_data_frame(log2(counts(ddo_4_or, normalized=TRUE)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
  as_data_frame(assay(vsd_4_or)[, 1:2]) %>% mutate(transformation = "vst"),
  as_data_frame(assay(rld_4_or)[, 1:2]) %>% mutate(transformation = "rlog"))
...
```{r}
meanSdPlot(assay(vsd_4_or))
```

```{r}
meanSdPlot(assay(rld_4_or))
```

```{r}
vsd_i_or <- vst(ddo_i_or, blind=FALSE)
rld_i_or <- rlog(ddo_i_or, blind=FALSE)
```

```{r}
ddo_i_or <- estimateSizeFactors(ddo_i_or)

df <- bind_rows(
  as_data_frame(log2(counts(ddo_i_or, normalized=TRUE)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
  as_data_frame(assay(vsd_i_or)[, 1:2]) %>% mutate(transformation = "vst"),
  as_data_frame(assay(rld_i_or)[, 1:2]) %>% mutate(transformation = "rlog"))
...
```{r}
meanSdPlot(assay(vsd_i_or))
```

```{r}
meanSdPlot(assay(rld_i_or))
```
...
```{r}
# save R DESeq Data Object
save(ddo_i_or, file = "../output/07_exploration/ddo_i_or.RData")

# save gene count matrix
save(gcm_filt_or, file = "../output/07_exploration/gcm_filt_or.RData")
...
```{r}
save(ddo_4_or, file = "../output/07_exploration/ddo_4_or.RData")

# save gene count matrix
save(gcm_filt_4_or, file = "../output/07_exploration/gcm_filt_4_or.RData")

#output metadata for other analyses 
write_csv(metadata_4_or, "../metadata/metadata_4_or.csv")
```

```{r}
save(ddo_9, file = "../output/07_exploration/ddo_9.RData")

# save gene count matrix
save(gcm_filt_9, file = "../output/07_exploration/gcm_filt_9.RData")
...
```{r}
save(ddo_14, file = "../output/07_exploration/ddo_14.RData")

# save gene count matrix
save(gcm_filt_14, file = "../output/07_exploration/gcm_filt_14.RData")
