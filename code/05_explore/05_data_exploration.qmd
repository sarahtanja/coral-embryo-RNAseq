...
#  "bioc::DESeq2"
#))
#install.packages("shinyjs") # for colorspace hclwizard
```
...
gcm_filt <- gcm %>%
  filter(rowSums(across(everything(), ~ . > 10)) >= 0.85 * ncol(gcm))
...
# Merge with metadata
pca_500 <- left_join(pca_500, metadata)
```
...
# Plot PCA
PCA_500 <- ggplot(pca_500, aes(PC1, PC2, color = stage, label = label)) +
  geom_point(size = 1) +
...
percentVar_500_or <- round(100*attr(pca_500_or, "percentVar"))

# Merge with metadata
pca_500_or <- left_join(pca_500_or, metadata_or)
...
pca_plot_spawn_night <- ggplot(
  ...
  geom_point(size = 2, stroke = 1.5, alpha = 0.9) +   # stroke controls outline width
...
```
::: callout-note
It seems like there may be clustering by spawn night, particularly within the cleavage stage.
:::
...
transposed matrix for PERMANOVA such that rows are samples with a 1 to 1 match to samples in our metadata dataframe.
...
`set.seed()` the permutation stream is reproducible so you (and future
...
Stage explains ~59% of total variance in gene expression patterns (stage: R² = 0.588, F = 40.67, p = 0.001), indicating strong statistically significant centroid separation. 

Leachate explains ~2.3% of total variance in gene expression patterns (leachate: R² = 0.023, F = 1.08, p = 0.344), indicating no significant global multivariate shift detectable at this scale. This result doesn't mean there are no effects, it means that any effects are likely subtle, gene-specific, stage-dependent, better captured by DE models than by ordination. 

The interaction between stage and leachate explained ~3.5% of total variance in gene expression patterns (stage:leachate: R² = 0.035, F = 0.809, p = 0.688), indicating no significant interaction effect on overall multivariate expression profiles, and no evidence that leachate reshapes expression profiles differently by stage at the global level.

35% residual variation unexplained by the model. 

The model revealed that developmental stage explained ~59% of the overall variance in gene expression ( R² = 0.59, F =
...
1.09, *p* = 0.338 ) nor the interaction between leachate and stage ( R²= 0.04, F = 0.81, *p* = 0.679 ) significantly influenced the
multivariate expression structure. Residual variation accounted for 35% of the total variance.
:::
...
```{r}
boxplot(bd_night, xlab = "Spawn night", ylab = "Distance to centroid")
```
::: callout-note

Principal component analysis revealed clustering of samples by developmental stage, with additional structure associated with spawn night. 

There is significant variation in dispersion among spawn nights (p = 0.012), i.e. within-group dispersion differs among spawn nights. This means groups differ in variability, not just location. 

- Spawn night groups differ in multivariate spread
- PCA showing tight vs diffuse clusters is consistent with this result

Homogeneity of multivariate dispersion differed significantly among spawn nights (betadisper permutation test: 𝐹= 4.56, p = 0.012, 999 permutations), indicating unequal within-group variability.

Accordingly, spawn night was treated as a blocking factor in PERMANOVA to account for differences in overall variability among consecutive spawning nights. Tests for homogeneity of multivariate dispersion indicated significant differences among spawn nights supporting the use of spawn night as a blocking factor.

Differences in dispersion among spawn nights may reflect variation in parental crosses, gametogenesis synchrony, maternal provisioning, consistent with known biological heterogeneity across consecutive spawning nights. 
...
```
```{r}
ggplot(as.data.frame(bd_leachate$distances), aes(x = grp_leachate, y = bd_leachate$distances)) +
  geom_boxplot() +
  xlab("Leachate") +
  ylab("Distance to centroid")
```
::: callout-note
Tests for homogeneity of multivariate dispersion indicated no significant differences among leachate treatments (betadisper permutation test: F=0.66, p=0.592, 999 permutations), suggesting comparable within-group multivariate variability across leachate levels. Accordingly, the absence of a significant PERMANOVA effect of leachate is unlikely to be driven by heterogeneity of dispersion.
:::
...
```{r}
ggplot(as.data.frame(bd_stage$distances), aes(x = grp_stage, y = bd_stage$distances)) +
  geom_boxplot() +
  xlab("Stage") +
  ylab("Distance to centroid")
```
...
::: callout-note
- Within-stage multivariate dispersion differs significantly
- Some stages are more variable than others in expression space

Therefore:
PERMANOVA significance for stage reflects both centroid separation AND differences in dispersion. But this does not negate the stage effect!

Tests for homogeneity of multivariate dispersion revealed significant differences in within-stage variability (betadisper permutation test: F=15.03, p=0.001, 999 permutations), suggesting comparable within-group multivariate variability across leachate levels. Accordingly, the absence of a significant PERMANOVA effect of leachate is unlikely to be driven by heterogeneity of dispersion.

Permutational multivariate analysis of variance (PERMANOVA) identified developmental stage as the dominant driver of multivariate expression differences (pseudo-F=40.67, R= 0.588, p = 0.001). Tests for homogeneity of multivariate dispersion further indicated significant differences in within-stage variability (betadisper permutation test: F=15.03, p=0.001), suggesting that the observed stage effect reflects a combination of strong centroid separation and differences in dispersion among stages.

Prawnchip stage embryos experience a bottleneck, or "selection event", where embryos that progress are characterized by reduced variability in gene expression profiles. In contrast, earlier cleavage stages and later larval stages exhibit greater heterogeneity in developmental timing and gene expression patterns, reflected in increased within-stage dispersion.

Despite differences in dispersion, the magnitude of the stage effect and clear separation observed in PCA indicate that developmental stage represents a robust and biologically meaningful source of multivariate variation.
:::
...
pheat_df <- as.data.frame(colData(vsd_or)[ , c("leachate", "stage", "spawn_night")])
legend_names_col = colnames(assay(vsd_or))
names(pheat_df)<- c("leachate", "stage", "spawn_night")
rownames(pheat_df) <- legend_names_col
```
...
# Save VST matrix `mat` for pheatmap final figure
saveRDS(mat, "../../output/05_explore/vst_matrix.rds")

# Save pca_500_or for final figure PCA
write_csv(pca_500_or, "../../output/05_explore/pca_500_or.csv")
saveRDS(pheat_df, "../../output/05_explore/pheat_annotation_df.rds")
```
