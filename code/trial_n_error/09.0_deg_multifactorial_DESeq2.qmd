...
res_14_high_int <- (results(dds_4, name = "hpf14.leachatehigh", alpha=0.05)
)

summary(res_4_high_int)
```
...
```{r}
# Automatically calculated by DESeq()
mcols(dds)$maxCooks

# Plot histogram of Cook's distances
hist(mcols(dds)$maxCooks, breaks = 100, main = "Max Cook's Distance per Gene")
