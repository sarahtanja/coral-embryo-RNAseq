...
## Test for each term of the model
...
nl_leachate2_sig <-
  topTags(
  nl_leachate2,
  n = (70 + 1599), # the number of up and down degs from is.de_nl_leachate2
  adjust.method = "BH",
  p.value = 0.05
  )

nl_leachate2_sig_geneids <- row.names(nl_leachate2_sig) #Output a list of geneids associated with sig leachate^2 effect

nl_leachate_sig <-
  topTags(
  nl_leachate,
  n = (1314 + 62), # the number of up and down degs from is.de_nl_leachate
  adjust.method = "BH",
  p.value = 0.05
  )
...
tab_exp_df$hpf_sig <-
  ifelse(tab_exp_df$geneid %in% nl_hpf_sig_geneids, "Yes", "No")
tab_exp_df$hpfleachate_sig <-
  ifelse(tab_exp_df$geneid %in% nl_hpfleachate_sig_geneids, "Yes", "No")
tab_exp_df$hpfleachate_2_sig <-
  ifelse(tab_exp_df$geneid %in% nl_hpfleachate2_sig_geneids, "Yes", "No")

# Create a binary variable related to up or down-regulation
up_genes <- filter(nl_hpf_sig$table, logFC > 0)
...
tab_exp_avg <- summarySE(
  ...
  groupvars = c("leachate",     "hpf",       "geneid", 
                "leachate_sig", "leachate_2_sig", "hpf_sig", "hpfleachate_sig", "hpfleachate_2_sig", "logFC_dir"),
  data = tab_exp_df
  )
...
write_csv(LRT_filt_df, "../output/09_multifactorial/LRT_filt_df.csv")
save(nl_leachate_2_int, file = "../output/09_multifactorial/nl_leachate_2_int.RData")
```

# Interaction

> How does gene expression vary between exposure times for genes exhibiting significant interactions between *leachate*2\^2 and time? Let's produce an exploratory plot of non-linear expression across hpf of exposure for such genes identified using edgeR alone.

```{r}
# Export diff expression data for transcripts with significant DE associated with interaction between leachate^2 and time
nl_leachate2_sig <- topTags(nl_leachate2, n = (70 + 1599), adjust.method = "BH",p.value = 0.05)
nl_leachate2_sig_geneids <- row.names(nl_leachate2_sig) #Output a list of geneids associated with sig leachate^2 x time interaction

# Filter down df for gene id's exhibit leachate significant effect in edgeR and significant likelihood of nl effect in LRT 
edgeR_interaction_df <- filter(tab_exp_avg, geneid %in% nl_leachate2_sig_geneids )
...
design_multi <-
  model.matrix(~ 1 + hpf + leachate + leachate:hpf) #Generate multivariate edgeR glm
...
dds <- DESeqDataSetFromMatrix(gcounts,
                              colData = metadata_or,
                              design = formula( ~ 1 + hpf + leachate + leachate_2 + hpf:leachate + hpf:leachate_2))
