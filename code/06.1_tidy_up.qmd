---
title: "Step 6: Tidy up Count Matrix"
subtitle: "Conduct prefiltering & identify outliers"
author: "Sarah Tanja"
date: 2024-11-18
date-modified: today
format: gfm
...
```{r}
metadata$embryonic_stage <- gsub(" ", "", metadata$development_stage)
head(metadata)
```
...
metadata$embryonic_stage<-as.factor(metadata$embryonic_phase)

metadata$embryonic_stage<-fct_relevel(metadata$embryonic_phase, "cleavage", "prawnchip", "earlygastrula")
...
```{r}
metadata <- metadata %>% dplyr::select(sample_no, sample_name, parents, group, treatment_group, hpf, embryonic_stage, pvc_leachate_level, hours_post_fertilization, pvc_leachate_concentration_mg.L)
