---
title: "Step 3: Align and assemble sequence reads to annotated *M. capitata* genome"
subtitle: "Using `HISAT2` & `StringTie`"
author: "Sarah Tanja"
date: 10/21/2024
date-modified: today
format:
...
This notebook will align trimmed M. capitata RNA-seq data to the M. capitata genome using hierarchical indexing for spliced alignment of transcripts `HISAT2` [@zhang_rapid_2021]. Followed by assembly with `StringTie` (Pertea et al. 2016, 2015) for transcript identification.
...
```{r, gtf-download, engine='bash', eval=FALSE}
cd ../../input/genome
...
```{r, gtf-checksum, engine='bash', eval=FALSE}
...
```{r, hisat-download, engine = 'bash', eval=FALSE}
# change to work in data genome directory
cd ../../input/hisat
...
```{r, hisat-checksum, engine = 'bash', eval=FALSE}
# change to work in data genome directory
cd ../../input/hisat
...
```{r, hisat-unzip, engine = 'bash', eval=FALSE}
cd ../../input/hisat
...
```{r bash-vars, engine='bash', eval=FALSE}
{
...
```{r gtf-gene-count-2, engine='bash'}
grep -c "gene" ../../input/genome/Montipora_capitata_HIv3.genes.gtf
```

# Extract exons

```{r exon, engine='bash', eval=FALSE}
/home/shared/hisat2-2.2.1/hisat2_extract_exons.py \
../../input/genome/Montipora_capitata_HIv3.genes.gtf \
> ../../output/03_align/exon.tab
```

```{r exon-head, engine='bash', eval=FALSE}
head ../../output/03_align/exon.tab
wc -l ../../output/03_align/exon.tab #count the number of files 
```

Exons are the coding regions of a gene that remain in the final mature mRNA after introns are removed during splicing. In the genome sequence, exons are separated by introns (non-coding stretches that get spliced out).
...
Our RNA-seq reads come from mRNA, so our reads will map to exons in the genome. HISAT2 needs to know where the exons are in the genome to align our reads correctly.

# Extract splice sites

```{r extract-splices, engine='bash', eval=FALSE}
...
```{r exon-head-2, engine='bash'}
head ../../output/03_align/splice_sites.tab
wc -l ../../output/03_align/splice_sites.tab #count the number of files 
```

Splice sites are the **boundaries between exons and introns** where splicing occurs during mRNA processing.These positions tell HISAT2 where reads might "jump" from one exon to another, because RNA-seq reads can span exon--exon junctions.

-   **Donor site (5′ splice site):** the beginning of an intron.

-   **Acceptor site (3′ splice site):** the end of an intron.

```         
DNA:   [EXON1]---intron---[EXON2]
Reads: [EXON1----EXON2]   (spanning across splice site)
```

HISAT2 can align reads in a "splice-aware" way. If a read doesn't align continuously, HISAT2 checks whether it could cross a known splice junction (from splice_sites.tab). This improves alignment accuracy, especially for reads that bridge exons. The helper scripts take a GTF annotation and produce:

-   exon.tab: A list of all exon coordinates (so HISAT2 knows where continuous alignments are expected).

-   splice_sites.tab: A list of exon--intron boundaries (so HISAT2 knows where gapped alignments are allowed).
...
```{r test-align-loop, engine='bash', eval=FALSE}
...
```{r hisat-loop, engine='bash', eval=FALSE}
for file in ../../output/02_qaqc/trimmed_reads/*[CLMH]4_R2_001.fastp-trim.fq.gz
do
...
```{r sam-to-bam, engine='bash', eval=FALSE}
...
::: callout-tip
StringTie makes the gtf files used as input to make the count matrix in the next step
:::
